...
class PinEntryEditText @JvmOverloads constructor(
    ...
    private lateinit var onPinCompleteListener: (CharSequence?) -> Unit
    ...
    fun setOnPinCompletedListener(listener: (CharSequence?) -> Unit) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        val colorFocused =
            if (isInEditMode) Color.GRAY else ContextCompat.getColor(context, R.color.teal_700)
        mColors[1] = colorFocused
        val colorUnfocused =
            if (isInEditMode) Color.GRAY else ContextCompat.getColor(context, R.color.teal_700)
        mColors[2] = colorUnfocused
        setBackgroundResource(0)
        mMaxLength = attrs?.getAttributeIntValue(XML_NAMESPACE_ANDROID, "maxLength", 4) ?: 4
    ...
    override fun onTextChanged(
        ...
        if (mLineCoords == null || !mAnimate) {
            if (this::onPinCompleteListener.isInitialized && text.length == mMaxLength) {
                onPinCompleteListener.invoke(text)
            }
            return
        }
        if (mAnimatedType == -1) {
    ...
    private fun animatePopIn() {
        ...
        if (text!!.length == mMaxLength && this::onPinCompleteListener.isInitialized) {
    ...
    private fun animateBottomUp(text: CharSequence, start: Int) {
        ...
        if (text.length == mMaxLength && this::onPinCompleteListener.isInitialized) {
            set.addListener(object : Animator.AnimatorListener {
                override fun onAnimationStart(animation: Animator) {}
                override fun onAnimationEnd(animation: Animator) {
                    onPinCompleteListener.invoke(getText())
                }
